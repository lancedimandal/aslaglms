...
$sqlDelete = "DELETE FROM activities WHERE activity_id = ?";
...
if (mysqli_stmt_execute($stmt)) {
    ...
    $response = array('status' => 'success', 'message' => 'Activity Deleted Successfully');
    echo json_encode($response);
} else {
    // Deletion failed
    // Return a JSON response with an error message
    $response = array('status' => 'error', 'message' => 'Activity Deletion Failed: ' . mysqli_error($connection));
    echo json_encode($response);
}
