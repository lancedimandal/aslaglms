...
if (!isset($_GET["id"])) {
    die("Error: Activity ID not provided in the URL.");
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $stmt_insert_question = $connection->prepare("INSERT INTO act_questions (activity_id, question_text) VALUES (?, ?)");

    if (!$stmt_insert_question) {
        die("Error: " . $connection->error);
    }

    $stmt_insert_question->bind_param("is", $activity_id, $question_text);
...
<div class="container mt-4">
    <div class="row">
        <div class="col-md-6">
            <h1>Add Question to Activity</h1>
        ...
        <div class="col-md-6 mt-10" style="margin-left: 0">
            ...
            <div> <h1 class="text-center">Activity Question List</h1></div>
            <?php
            require 'config/connection.php';
            ...
            // Fetch the total number of questions in the database for the specified activity
            $totalQuestionsSql = "SELECT COUNT(*) as total FROM act_questions WHERE activity_id = $activityId";
            ...
            // Fetch activity questions from the database with pagination
            $sql = "SELECT * FROM act_questions WHERE activity_id = $activityId LIMIT $offset, $questionsPerPage";
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                if ($current_page > 1) {
                    echo '<li class="page-item"><a class="page-link" href="?id=' . $activityId . '&page=' . $prevPage . '">Previous</a></li>';
                }

                echo '<li class="page-item"><span class="page-link">Page ' . $current_page . ' of ' . $totalPages . '</span></li>';

                if ($current_page < $totalPages) {
                    echo '<li class="page-item"><a class="page-link" href="?id=' . $activityId . '&page=' . $nextPage . '">Next</a></li>';
                }
                echo '</ul>';
