...
$sqlDelete = "DELETE FROM quiz WHERE quiz_id = ?";
...
if (mysqli_stmt_execute($stmt)) {
    ...
    $response = array('status' => 'success', 'message' => 'Quiz Deleted Successfully');
    echo json_encode($response);
} else {
    // Deletion failed
    // Return a JSON response with an error message
    $response = array('status' => 'error', 'message' => 'Quiz Deletion Failed');
    echo json_encode($response);
}
